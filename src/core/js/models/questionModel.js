define([
  ...
  var QuestionModel = ComponentModel.extend({
    ...
    setupModel: function(...args) {
      ComponentModel.prototype.setupModel.call(this, ...args);
     ...
     * @returns {Array}
     */
    getAttemptState: function(object = this.toJSON()) {
     ...
     * @returns {Object}
     */
    getAttemptObject: function(state = this.getAttemptState()) {
      const trackables = this.trackable();
      const types = this.trackableType();
      trackables.find((name, index) => {
        if (name !== '_attemptStates') return;
     ...
     */
    setAttemptObject: function(object) {
     ...
     * @param {Object} [object] JSON object representing the component state. Defaults to current JSON.
     */
    addAttemptObject: function(object = this.getAttemptObject()) {
